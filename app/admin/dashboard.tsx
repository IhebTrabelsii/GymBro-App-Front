...
export default function AdminDashboard() {
  ...
  const loadUserData = async () => {
    ...
    const fetchStats = async () => {
      try {
        const response = await fetch('http://192.168.100.143:3000/api/admin/stats');
        const data = await response.json() as { users?: number; workouts?: number; news?: number; error?: string };
        if (response.ok) {
    ...
    >
        ...
        <View style={styles.contentWrapper}>
              ...
              >
                <Animated.View style={animatedStyle}>
                  <TouchableOpacity
                    ...
                    onPress={() => Alert.alert('Info', 'Categories management coming soon!')}
                    style={[
                      styles.iconButton,
                      {
                  ...
                  >
                    ...
                    <Text style={[styles.buttonText, { color: primaryColor }]}>Categories</Text>
                  </TouchableOpacity>
                </Animated.View>
          ...
          >
            <View style={styles.gridContainer}>
              {[
                { title: 'Total Users', value: stats.users, icon: 'people', color: primaryColor },
                { title: 'Workout Plans', value: stats.workouts, icon: 'barbell', color: primaryColor },
                { title: 'News Articles', value: stats.news, icon: 'newspaper', color: primaryColor },
              ].map((stat, index) => (
                <Animated.View
                  key={index}
      ...
      >
        <TouchableOpacity
          ...
          activeOpacity={0.9}
          onPress={() => Alert.alert('Quick Action', 'What would you like to do?')}
        >
